...
public class AIMLMap extends HashMap<String, String>
{
    ...
    Brain brain;
        ...
        */
    public AIMLMap (String name, Brain brain) {
        super();
        this.brain = brain;
        this.mapName = name;
    }
    ...
    public String put(String key, String value) {
       ...
       public  void writeAIMLMap () {
           ...
           try{
               // Create file
               FileWriter fstream = new FileWriter(brain.maps_path+"/"+mapName+".txt");
    ...
    public int readAIMLMapFromInputStream(InputStream in, Brain brain)  {
        int cnt=0;
        ...
        */
    public int readAIMLMap (Brain brain) {
        int cnt = 0;
        if (MagicBooleans.trace_mode) System.out.println("Reading AIML Map " + brain.maps_path+"/"+mapName+".txt");
        try{
            // Open the file that is the first
            // command line parameter
            File file = new File(brain.maps_path+"/"+mapName+".txt");
            if (file.exists()) {
                FileInputStream fstream = new FileInputStream(brain.maps_path+"/"+mapName+".txt");
                // Get the object
                cnt = readAIMLMapFromInputStream(fstream, brain);
                fstream.close();
            }
            else System.out.println(brain.maps_path+"/"+mapName+".txt not found");
        }catch (Exception e){//Catch exception if any
            System.err.println("Error: " + e.getMessage());
        }
        return cnt;
