...
public class AIMLSet extends HashSet<String>
{
     ...
     */
    public AIMLSet (String name, Brain brain) {
        super();
        this.brain = brain;
    ...
    public  void writeAIMLSet () {
        ...
        try{
            // Create file
            FileWriter fstream = new FileWriter(brain.sets_path+"/"+setName+".txt");
    ...
    public int readAIMLSetFromInputStream(InputStream in, Brain brain)  {
    ...
    public int readAIMLSet (Brain brain) {
        int cnt=0;
        if (MagicBooleans.trace_mode) System.out.println("Reading AIML Set "+brain.sets_path+"/"+setName+".txt");
        try{
            // Open the file that is the first
            // command line parameter
            File file = new File(brain.sets_path+"/"+setName+".txt");
            if (file.exists()) {
                FileInputStream fstream = new FileInputStream(brain.sets_path+"/"+setName+".txt");
                // Get the object
                cnt = readAIMLSetFromInputStream(fstream, brain);
                fstream.close();
            }
            else System.out.println(brain.sets_path+"/"+setName+".txt not found");
        }catch (Exception e){//Catch exception if any
            System.err.println("Error: " + e.getMessage());
        }
        return cnt;
